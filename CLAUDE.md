...
1. Accessibility pass — finish the last piece: audit images across the site (`ProductCard`, `ProductDetail`, `HeroSection`, `BrandStory`, `Cart`, `FilterSidebar`, `FormField`) and mark purely decorative ones `aria-hidden="true"` (or `alt=""`); content images already have real `alt` text.
2. Remaining optional polish from the plan's Phase 7: consider deployment (Vercel/Netlify + Render/Railway, Atlas is already in place). `README.md` is done (2026-09-04) — real feature list, stack, structure, and setup steps replaced the default Vite template.
